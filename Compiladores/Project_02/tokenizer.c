    ...
    for(i = 0; i < MAX_NUMLEN; ++i) {
      if(!isdigit(ptr[i])) {
        if(i > 0) {
          ...
          DEBUG_PRINTF("get_next_token: error [numero demasioado pequeño]\n");
          return TOKENIZER_ERROR;
        }
      }
      if(!isdigit(ptr[i])) {
        DEBUG_PRINTF("get_next_token: error [numero malformado]\n");
        return TOKENIZER_ERROR;
      }
    }
    DEBUG_PRINTF("get_next_token: error [numero demasiado grando]\n");
    return TOKENIZER_ERROR;
  } else if(singlechar()) {
    nextptr = ptr + 1;
    return singlechar();
...
void tokenizer_next(void) {
  ...
  DEBUG_PRINTF("siguiente token: %p\n", nextptr);
  ptr = nextptr;
  ...
  DEBUG_PRINTF("siguiente token: '%s' %d\n", ptr, current_token);
  return;
}
